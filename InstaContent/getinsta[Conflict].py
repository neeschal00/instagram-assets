...
class InstaContent:
    ...
    def downloadPost(self,url:str,filename = None) -> bool:
        ...
        if not _url.path.startswith('/p/'):
            ...
            self.instaS.headers.update(self.headers)
    ...
    async def get_single_media(self,session,url:str,position:int,is_video:bool=False): #work with single media file
        async with session.get(url) as response:
            ...
            if is_video:
                print('true')
                InstaContent.writeVideo('hello'+str(position),filecontent)
            else:
                print('false is image')
                InstaContent.writeImage('hellow'+str(position),filecontent)
...
if __name__ == "__main__":
    ...
    obj.downloadPost('https://www.instagram.com/p/CP8ay28HgQ-/',filename=None)
